...
class attandence_check():
    # init
    def __init__(self, info_path, outlier, update_date=None):
        ...
        self.worksheet_date = self.get_updatesheet(update_date)
    ...
    def update_total(self, update_date = None):
        ...
        self.attandence_sheet.update_cell(sheet_update, 't', update_date)
        self.attandence_sheet.update_cell(sheet_update, 'h', update_date)
    ...
    def update_state(self, update_date = None):
        # print(update_date)
        attand = self.update_leave_or_work('g', update_date)
        leave = self.update_leave_or_work('l', update_date)
        print('update state')
        ...
        for user in attand:
            if attand[user] == '-':
                ...
                sheet_update[user] = '○'
        self.attandence_sheet.update_cell(sheet_update, 's', update_date)
        
           
    def reminder(self, time):
        """
            time: str, time messages 11pm, 01am, 02am, 03am
        """
        ...
        for user in not_leave_list:
            self.slack.caht_DM(not_leave_list[user], self.texts[time])
            
    
    def get_updatesheet(self, update_date):
        if update_date is not None:
            y = update_date[:4]
            m = update_date[4:6]
